...
c
	...
	character*5 listfilnam1,listfilnam2
	character*80 dalidatpath_1,dalidatpath_2
	logical lpreali,lfirstonly
c
	lpreali=.true.
	...
	write(*,*) 'enter lfirstonly'
	read(*,*) lfirstonly
	write(*,*) 'got: ',lfirstonly
c
	listfilnam1='list1'
	listfilnam2='list2'
	call main(.true.,.true.,.true.,1,listfilnam1,listfilnam2,
     $		lfirstonly,lpreali,dalidatpath_1,dalidatpath_2)
c
510	format(a80)
c
	end
...
c----------------------------------------------------------------------
c
	subroutine main(ldb,lnul,lseqtl,startsize,listfilnam1,listfilnam2,
     $		lfirstonly,lpreali,dalidatpath_1,dalidatpath_2)
	implicit none
	include 'parsizes.for'
	logical ldb,lnul,lseqtl,lfirstonly,lpreali
	integer startsize
	character*5 listfilnam1,listfilnam2
	character*80 dalidatpath_1,dalidatpath_2
c
	...
	integer segment2(maxres2),nseg2,nres2,dist2sum(maxres2,0:maxres2)
	integer dist1sum(maxres1,0:maxres1),minseglen(maxseg),ngap(maxseg)
	...
	integer ss(maxseg,maxseg),domns(maxdom),domseglist(maxseg,maxdom)
	logical lfix(maxseg,maxdom),ldom(0:maxdom),lexdone(maxseg,maxseg)
	logical lfix1(maxseg,maxdom)
...
c
	...
	nprot2=0
	if(ldb) then
		call getlist(listfilnam1,90,list1,nprot1)
c	        write(*,*) (list1(i),i=1,nprot1)
		call getlist(listfilnam2,90,list2,nprot2)
...
10	str1='structure '
	lskipflag=.false.
	call nextprotein(1,string,str1,ldb,iprot1,nprot1,list1,ier)
	if(ier.ne.0) goto 99
	call getstructure1(nseg,segmentrange,na1,nb1,1,1,ndom,node_child,
     $		dist,nres1,string,upper,lower,ss,checkrange,secstr,lfix,lfix1,
     $		cut,domns,domseglist,ier,checkx,dist1sum,segmentrange0,ldb,
     $		dalidatpath_1)
	if(lskipflag) goto 10
	if(ier.ne.0) goto 10
...
c
	iprot2=0
	str1='sequence  '
20	lskipflag=.false. ! initialize for new pair
	call nextprotein(6,string,str1,ldb,iprot2,nprot2,list2,ier)
c	write(*,*) string
	write(97,*) string,ier
	if(ier.ne.0) goto 10
c	write(*,*) 'pair: ',string
...
c	write(*,*) ier
	if(ier.ne.0) goto 20
	if(lskipflag) goto 20
	call initlexdonestart(nseg,ss,start,lexdone)
	call align(ndom,node_child,nres2,lnul,nseg,cut,ldom,
     ...
     $		minseglen,ngap,segmentrange0,ldb,lfirstonly,lpreali)
	goto 20
c
99	return
	end
...
c	write(*,*) 'getprotein1 done',nseg,ndom,nseg,na1,na2,nb1,nb2
	if((nseg.le.2).or.(ndom.lt.nseg).or.(na1*na2+nb1*nb2.eq.0)) flag=1
	write(*,*) flag
	if(flag.ne.0) return
...
10	open(iunit,file=filnam,status='old',err=99)
	call readproteindata(nres,nseg,na,nb,secstr,segmentrange,
...
c		!minimal distance=1, maximal distance=maxdist
                dist(i,i)=1
                do j=i+1,nres1
                  dist(i,j)=distance(ca(1,i),ca(2,i),ca(3,i),ca(1,j),
     $                          ca(2,j),ca(3,j))
...
20	call readproteindata(nres,nseg,na,nb,secstr,segmentrange,
     $		checkrange,checkx,ca,ndom,node_type,node_child,domns,
     $		domseglist,iunit)
	close(iunit)
c
...
c-------------------------------------------------------------------------------
c
	subroutine readproteindata(nres,nseg,na,nb,secstr,segmentrange,
     $		checkrange,checkx,ca,ndom,node_type,node_child,domns,
     $		domseglist,iunit)
	implicit none
	include 'parsizes.for'
