...
c     $		(secstr(i),i=1,nseg),(secstr2(i),i=1,nseg2),ldb,lfirstonly
	do idom=1,ndom
		cut(idom)=cut0(idom)
	end do
	laststart=0
...
c
c	write(*,*) 'dostack',scorecutoff,top,chunk,ns,firstest
	do is=1,ns
		ali(seglist(is))=0
	end do
...
c
			...
			write(95,510) string,idom,est,ns,
     $				(a1(is),a2(is),is=1,ns),(b1(is),b2(is),is=1,ns)
		end if
	else
		stop 'unknown flag returned from getnextbest'
	end if
...
c
	...
	include 'parsizes.for'
	integer idom,ali_nali(maxdom),ali_start(maxdom),ali_laststart
	integer ali_save(1000000),trans(maxres0,maxseg),
     $ ns,seglist(maxseg)
	integer ali(maxseg)
c
