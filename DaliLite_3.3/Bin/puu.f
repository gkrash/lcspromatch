...
c
	...
	character*60 basedefault,basestring,line,emptystring,versionstring
	integer*4 seed,i
        logical lhetatm,lsplit,lsource,lauthor,lcompnd,lheader
c
c
c
	versionstring='2.0 (December 1994)' ! first round = chain-subunits
c
c
c
...
c
	...
	call defaults(dssplist,pdblist,breakdist,minseglen,hbond,compact,
     $		taucutoff,seed,treefilename,prettyfilename,
     $          defaultsfilename,
     $		lhetatm,lsplit,printdepth,basedefault,subunitfilename)
c
c	initialization
c
...
c	input loop
c
10      write(*,*) ' enter code+chainid ? (END to quit) '
        read(*,500) code,chainid
        if(code(1:3).eq.'END') goto 999
	call doprotein(code,chainid,nsearch,search,dssplist,lhetatm,pdblist,
...
c-----------------------------------------------------------------------
c
	subroutine doprotein(code,chainid,nsearch,search,dssplist,lhetatm,
...
c
	logical lonechain
	integer nchain,seg(2,maxres),restoseg(maxres),natom(100)
	character oldc
	real sscont(100,100)
...
c
	nres0=0
	call setup_protein(code,chainid,nres0,natom0,occx0,npoint0,point0,
...
c
		call initialize_one(nchain,ndom,nstack,stack,set,node_range,
     $			node_nseg,node_child,node_tau)
...
c		write(*,*) (set(i),i=1,nres0)
		...
		chain(1)=chainid
		subunit(1)=1
		call initialize_one(nres0,ndom,nstack,stack,set,node_range,
     $			node_nseg,node_child,node_tau)
	end if
c
c	output subunit decomposition
c
	write(33,550) code,nchain,(subunit(i),i=1,nchain),
     $		(chain(i),i=1,nchain)
c
c	hack
c
...
c
	call patch(ndom,nres0,node_child,node_parent,node_nseg,node_range,
     $		node_size)
c	write(*,*) 'patch done'
c
c	tertiary contacts
...
c	beta-sheets
c
	call betasheets(ndom,node_child,node_nbp,bp0,node_range,node_nseg)
c	write(*,*) 'beta-sheets done'
c
	call findstrucdoms(ndom,node_child,node_size,node_tau,node_tert,
     $		node_nbp,compact,taucutoff,lsplit,node_type,80)
c	write(*,*) 'findstrucdoms done'
c
c###	write some output !
c
...
550	format('PDBID     ',a4,/'NCHAIN    ',i5,/'UNITID    ',300i5,
     $		/'CHAINID   ',300(4x,a1),/'//')
...
c-----------------------------------------------------------------------
c
	subroutine initialize_one(nres0,ndom,nstack,stack,set,node_range,
     $		node_nseg,node_child,node_tau)
	implicit none
	include 'sizes-puu.for'
...
c-----------------------------------------------------------------------
c
	subroutine dostack(nres0,ca0,occx0,npoint0,point0,natom0,breakdist,
     $		minseglen,seed,node_range,node_nseg,node_child,node_tau,
     $		ndom,cont,nstack,stack,set)
	implicit none
	include 'sizes-puu.for'
...
c
	  ...
	  call selectset(iset,set,ca0,occx0,npoint0,point0,occx,npoint,point,
     $		nres0,natom0,ca,cont,nres,n,ltest,
     $		resix,breakdist,minseglen)
c	  write(*,*) 'selectset done',nstack
c
...
c
	  ...
	  call eigen(bestcut,bottom,taubest,nres,n,occx,npoint,point,cont,
     $			ltest,minseglen,seed)
c	  write(*,*) 'eigen done',nstack
c	  write(*,*) code,chainid,ndom,taubest,(resix(bestcut(i)),i=1,bottom)
c
c	  remember domains: quit if taubest=0, i.e. no subdivision was found
c
	  if(taubest.gt.0.0) call savecut(iset,bestcut,bottom,nres,set,resix,
...
c
	...
	include 'sizes-puu.for'
	integer nsplitsheet
	integer inode,jnode,node_range(maxres,2,maxnseg),node_nseg(maxres)
	integer bp0(2,maxres)
c
...
c-------------------------------------------------------------------------------
c
	subroutine contstren(occx,npoint,point,node_range,node_nseg,natom0,
     $		ndom,nres,node_tert,cont,four)
	implicit none
	include 'sizes-puu.for'
	...
	real cont(500,500)
c
	integer i,j,k,l,iseg,natom,n(maxres),ires,jres,iset,jset,set(maxres),ix
	real x
        logical lhetatm,lsplit,lsource,lauthor,lcompnd,lheader
c
	n(1)=natom0(1)
...
c
	call correspondence_shortlist(nres,nres,occx,point,npoint,x,r2,seed,2)
c
...
c-----------------------------------------------------------------------
c
	subroutine definesets(x,nres,icut,k,minseglen,ltest,zerolevel,ei,
     $		choice)
	implicit none
	include 'sizes-puu.for'
...
c	write(*,*) nres,' residues in set ',iset
c
...
c------------------------------------------------------------------------------
c
	subroutine setup_protein(code,chainid,nres,natom0,occx,npoint,point,
...
c
	write(*,*) ' Reading coordinates from PDB file'
        pdbfilnam=findfile(code,pdblist,90)
...
c
	...
	write(*,*) ' Reading DSSP file'
        call readdssp1(findfile(code,dssplist,90),chainid,90,
...
530	format('PARAMETER pdbreader = ',a11,'; depth =',i5,'; seed = ',i20)
c
	return
	end
c
...
510	format(i5,2x,<maxres+1>a1,f5.1,2x,a3,i5)
...
c
	...
	nstrucdom=0
	do j=1,ndom
	   if(node_type(j).eq.'*'.or.node_type(j).eq.'=') nstrucdom=nstrucdom+1
	end do
...
c-------------------------------------------------------------------------------
c
	subroutine findstrucdoms(ndom,node_child,node_size,node_tau,node_tert,
     $		node_nbp,compact,taucutoff,lsplit,node_type,minsize)
	implicit none
	include 'sizes-puu.for'
