...
c	---------------------------------------------------------------------
c
	subroutine preparoutput(ali1i,ali1,blocksize,nres1,d1,d2,ca1,ca2,
...
c
	...
	integer nres1,blocksize,tsil(maxres),alilen(popsize),tsil2(maxres)
	real ca1(3,maxres),ca2(3,maxres)
	integer*2 d1(maxres,maxres),d2(maxres,maxres),ali1(maxres0,popsize)
	integer*2 ali1i(maxres,popsize)
	real rms(popsize),avscore(maxres0,popsize)
c
...
c	---------------------------------------------------------------------
c
	subroutine output(code1,chainid1,code2,chainid2,nres1i,nres2i,seq1,
     $		seq2,struc1,struc2,resno1,resno2,blocksize,hdr1,hdr2,ca1,ca2,
     $		relacc1,relacc2,d1,d2,ali1i,score,nres0,tsil,nres02,tsil2)
	implicit none
	include 'gagasizes.for'
	character*4 code1,code2
	character chainid1,chainid2
	integer nres1,nres2,blocksize,relacc1(maxres0),relacc2(maxres0)
	character struc1(maxres0),struc2(maxres0),seq1(maxres0),seq2(maxres0)
	character*4 resno1(maxres0),resno2(maxres0)
	character*80 hdr1,hdr2
	real ca1(3,maxres0),ca2(3,maxres0)
	integer*2 d1(maxres,maxres),d2(maxres,maxres),ali1i(maxres,popsize)
...
c
	lverb=.false.
	write(*,*) code1,chainid1,code2,chainid2,nres1i,nres2i
	write(*,*) nres0,nres02
	write(*,300) (ali1i(i,1),i=1,nres1i)
...
c	open(91,file=filnam,status='new')
	write(91,510) code1,chainid1,hdr1
	write(91,510) code2,chainid2,hdr2
...
!			if(relacc2(k1).le.corecutoff) l2=l2+1
     ...
     $				nide=nide+1
		...
		summary(1,i)=lenali
	if(ldebug)write(*,*) summary(10,i),summary(11,i),lenali,l1,
     $		lcore1,l2,lcore2
	end do
...
c	call printsummary(score,rms,summary,d)
	if(ldebug)write(*,*) ' dccp 8'
	do ind=1,popsize
	  i=d(popsize-ind+1)
	  if(score(i).gt.0.0) write(91,505) ind,score(i),rms(i),
...
c	call printblocks
	...
	if(ldebug)write(*,*) ' dccp 10'
	do ind=1,printpopsize
	  i=d(popsize-ind+1)
	  if(score(i).gt.0.0) then
		call getali0(ali1,i,a1,blocksize,nres1)
		write(91,530) ind
		write(91,540) (ib1(j,ind),ib2(j,ind),j=1,nblock(ind))
		write(91,540) (a1(ib1(j,ind)),a1(ib2(j,ind)),j=1,nblock(ind))
	  end if
	end do
	lali1=.false.
...
!		call getali0(ali1,i,a1,blocksize,nres1)
!		write(91,550) ind
!		write(91,560) (resno1(ib1(j,ind)),resno1(ib2(j,ind)),
!     $			j=1,nblock(ind))
!		write(91,560) (resno2(abs(a1(ib1(j,ind)))),
...
c !!! no sequence output !
	if(lali.and.lali1)then
	  do i=1,nres1,60
		k=min(59,nres1-i)
		write(91,600) i,i+k,resno1(i),resno1(i+k)
		write(91,590) (struc1(i+j),j=0,k)
		write(91,570) code1,chainid1,(seq1(i+j),j=0,k)
		do ind=1,printpopsize
		  l=d(popsize-ind+1)
     ...
     $						id(j+1)='|'
			...
			write(91,590) (id(j),j=1,k+1)
			write(91,580) code2,chainid2,ind,(se(j),j=1,k+1)
			write(91,590) (st(j),j=1,k+1)
		  end if
		end do
	  end do
	end if
...
c !!! no pretty !
	if(lpretty) then
     ...
     $			resno1,resno2,code1,code2,i,avscore)
	  end if
	end if
c	close(91)
...
c
	subroutine pretty(a1,seq1,seq2,struc1,struc2,nres1,nres2,
     $		resno1,resno2,code1,code2,ind,avscore)
	implicit none
	include 'gagasizes.for'
	integer a1(maxres),nres1,nres2,ind
...
c
	...
	do i=1,pos,60
		...
		if(j.le.k) i2=x2(i+j)
		write(91,500) (line(i+j,1),j=0,k)
		if(i1.gt.0) r=resno1(i1)
		if(i1.eq.0) r='    '
		write(91,510) code1,r,(line(i+j,2),j=0,k)
		write(91,500) (line(i+j,3),j=0,k)
		if(i1.gt.0) r=resno2(i2)
		if(i1.eq.0) r='    '
		write(91,510) code2,r,(line(i+j,4),j=0,k)
		write(91,500) (line(i+j,5),j=0,k)
		write(91,*) ' '
	end do
500	format(10x,60a1)
...
c
	...
	do ind=1,popsize
	  i=d(popsize-ind+1)
	  if(score(i).gt.0.0) write(*,500) ind,score(i),rms(i),
     $		(summary(j,i),j=1,10)
	end do
500	format(i4,7x,f7.1,f4.1,6i4,2i2,2i3)
