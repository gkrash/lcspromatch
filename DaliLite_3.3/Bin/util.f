...
c
        ...
        l2=i
        l3=len(ext)
c        write(*,*),'constructfilnam: |',cd,'|',path,ext,l1,l2,l3
        do i=1,80
                constructfilnam(i:i)=' '
        end do
...
99      write(*,*) 'ERROR opening filnam=',filnam
	stop "FATAL ERROR in getdata: Can't open DAT file "
...
c-----------------------------------------------------------------------
c
	subroutine getdssp(filnam,chainid,iunit,maxres,header,compnd,source,
...
c
	...
	data aasymbol/'ARNDCEQGHILKMFPSTWYVBZXXXXXXXXXXXXXXXX--CCCCIPPPW-XM'/
c
	integer i,k
c
	aminoacid(1:60)=
...
100	read(iunit,500,end=199) line
	...
	if(line(1:6).eq.'ATOM  '.or.(line(1:6).eq.'HETATM'.and.lhetatm)) then
		c=line(22:22)
		if(chainid.ne.'*') then
